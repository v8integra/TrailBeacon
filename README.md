...
- The minimap button and the AddOn list were never actually wired up to use the final icon art (`Media/Icon/TrailBeaconIcon.tga`) — still referenced a stock Blizzard icon path that may not even resolve in this build (hence the blank minimap circle). Fixed, and added `## IconTexture` to the TOC for the AddOn list icon.
...
Keep testing and reporting anything that still looks wrong — particularly the tracking arrow's rotation, which depends on WoW's world-coordinate axis convention and `GetPlayerFacing()`'s bearing convention (see the `ROTATION_SIGN`/`ROTATION_OFFSET` comment at the top of [Modules/TrackingArrow.lua](Modules/TrackingArrow.lua) if that needs correcting).
